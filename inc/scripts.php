...
function tr_grabber_init_print() {
    if ( is_admin() ) {
        ...
        $trg = get_option('tr_grabber');
        $trgu = unserialize($trg);
    
        $pq = array_key_exists( 'post_quality', $trgu ) ? $trgu['post_quality'] : 'none';
        $pl = array_key_exists( 'post_language', $trgu ) ? $trgu['post_language'] : 'none';
        ...
        wp_enqueue_script( 'tr-grabber-admin', TR_GRABBER_PLUGIN_URL."assets/js/admin.js", array('jquery', 'jquery-ui-sortable', 'wp-color-picker'), filemtime(TR_GRABBER_PLUGIN_DIR."assets/js/admin.js"), true);
