...
function save_post_movies($post_id) {
    ...
    if ( isset($_POST['trgrabber_id']) ) {
        ...
        if( isset($grabber['title']) && isset($grabber['overview']) ){
        ...
        } else if( isset($grabber['title']) && !isset($grabber['overview']) ) {
            wp_update_post( array( 'ID' => $post_id, 'post_status' => TR_GRABBER_POST_STATUS, 'post_title' => isset($grabber['title']) ? stripslashes($grabber['title']) : '', 'post_name' => isset($grabber['title']) ? stripslashes($grabber['title']) : '') );
        } else if( !isset($grabber['title']) && isset($grabber['overview']) ) {
            wp_update_post( array( 'ID' => $post_id, 'post_status' => TR_GRABBER_POST_STATUS, 'post_content' => isset($grabber['overview']) ? stripslashes($grabber['overview']) : '' ) );
        } else {
            wp_update_post( array( 'ID' => $post_id, 'post_status' => TR_GRABBER_POST_STATUS));
        }
        ...
        if( isset( $grabber['release_date'] ) ) {
           $a  = explode('-',$grabber['release_date']);

            wp_set_object_terms($post_id, $a[0], 'annee');
        }
        ...
        if( TR_GRABBER_UPLOAD_IMAGES == 1 ) {
            ...
            if( isset( $grabber['poster_path'] ) ) {
                ...
                if( TR_GRABBER_LANG == 'zh-CN' or TR_GRABBER_LANG == 'ar-AR' or TR_GRABBER_LANG == 'ko-KR' ) {
                    $ainp = $grabber['original_title'];
                } else {
                    $ainp = $grabber['title'];
                }

                $image_url = 'https://image.tmdb.org/t/p/w780/'.$grabber['poster_path'];
                ...
                $title_my_image = $title_img.'-'.$post_id.'-poster.jpg';

                $myimage        = get_page_by_title($title_my_image, OBJECT, 'attachment');

                //copy( esc_url_raw( 'https://image.tmdb.org/t/p/w780/'.$grabber['poster_path'] ), $upload_dir['path'].'/'.$title_img.'-'.$post_id.'-poster.jpg' );

                if(!$myimage){
                    $attachment = array(
                        //'guid' => $upload_dir['path'].'/'.sanitize_title($ainp).'-'.$post_id.'-poster.jpg', 
            ...
            if( isset( $grabber['backdrop_path'] ) ) {
                ...
                if( TR_GRABBER_LANG == 'zh-CN' or TR_GRABBER_LANG == 'ar-AR' or TR_GRABBER_LANG == 'ko-KR' or TR_GRABBER_LANG == "bs-BS" or TR_GRABBER_LANG == "bg-BG" or TR_GRABBER_LANG == "hr-HR" or TR_GRABBER_LANG == "cs-CZ" or TR_GRABBER_LANG == "da-DK" or TR_GRABBER_LANG == "he-IL" or  TR_GRABBER_LANG == 'el-GR' or TR_GRABBER_LANG == 'lb-LB' or TR_GRABBER_LANG == 'lt-LT' or TR_GRABBER_LANG == 'fa-IR' or TR_GRABBER_LANG == 'ro-RO' or TR_GRABBER_LANG == 'ru-RU' or TR_GRABBER_LANG == 'sk-SK' or TR_GRABBER_LANG == 'th-TH' or TR_GRABBER_LANG == 'tr-TR' or TR_GRABBER_LANG == 'tw-TW' or TR_GRABBER_LANG == 'uk-UA' or TR_GRABBER_LANG == 'vi-VN' ) {
                    $ainpb = $grabber['original_title'];
                } else {
                    $ainpb = $grabber['title'];
                }
             
                if ( is_rtl() ) { $title_backdrop = md5(sanitize_title($ainpb)); }else{ $title_backdrop = sanitize_title($ainpb); }
        ...
        $array_post_meta = array(
            ...
            TR_GRABBER_FIELD_RATING => isset($grabber['vote_average']) ? $grabber['vote_average'] : '',
    ...
    if( empty($_POST['trgrabber_id']) ) {
        ...
        $array_post_meta = array(
            ...
            TR_GRABBER_FIELD_RATING => isset($_POST['rating']) ? $_POST['rating'] : '',
    ...
    if( !empty($_POST['trgrabber_id']) ) {
        try { 
            cmcManagerWp::updateMovie($post_id, true);
        } catch(Exception $e){
