jQuery(document).ready(function($) {
    ...
    $("body").on("click", "#trgrabber_addlink", function(e){
        ...
        let pq_select = TrGrabber.pq;
        let pl_select = TrGrabber.pl;
        var clone = $( ".tr-grabber-row:first" ).clone().insertBefore( ".tr-grabber-row:first" );
        console.log(pq_select, pl_select);
        if(pl_select != 0){
            $( ".tr-grabber-row:first select:eq(0) option[value="+pl_select+"]" ).prop("selected", true);
        } else { $( ".tr-grabber-row:first select:eq(0) option" ).prop("selected", false);  }

        if(pq_select != 0){
            $( ".tr-grabber-row:first select:eq(1) option[value="+pq_select+"]" ).prop("selected", true);
        } else {
            console.log('no existe');
            $( ".tr-grabber-row:first select:eq(1) option" ).prop("selected", false);
        }
